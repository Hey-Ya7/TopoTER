...
class EspTop (X : Type*) where
  ...
  union_ouvert {ι : Type u_1} {u : ι → Set X} (hu : ∀ i, est_ouvert (u i)) :
    est_ouvert (⋃ i, u i)
...
lemma EspTop.bunion_ouvert {ι : Type u_1} {u : ι → Set X} {I : Set ι} (h : ∀ i ∈ I,
...
lemma ouvert_ssi_vois (s : Set X) : est_ouvert s ↔ ∀ x ∈ s, est_vois x s := by
  ...
  · intro h; choose! u hu using h
    have union : s = ⋃ x ∈ s, u x := by
      ...
      apply Iff.intro
      · intro hx; use x; apply And.intro
        · exact hx
...
lemma EspTop.inter_est_ferme {ι : Type u_1} {u : ι → Set X} (hu : ∀ i, est_ferme (u i)) :
    est_ferme (⋂ i, u i) := by
  simp only [est_ferme]
  rw [compl_iInter]
